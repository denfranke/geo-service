...
class MessageSenderTest {
    @Test
    @DisplayName("Проверяем, что сообщение отправляется на русском языке, если IP в России")
    ...
    @Test
    @DisplayName("Проверяем, что сообщение отправляется на английском языке, если IP в США")
    void sendMessageUSA() {
        Map<String, String> map = new HashMap<>();
        GeoService geoService = Mockito.mock(GeoService.class);
